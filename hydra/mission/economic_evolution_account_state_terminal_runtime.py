...
TERMINAL_VERDICT_RELATIVE_PATH = Path(
    "WORM/economic-evolution-account-state-router-0011-verdict-2026-07-14.json"
)
TERMINAL_VERDICT_SHA256 = (
    "4b66f4377a213613707eb4797fd923a8a832c3523057208354bec1e0d6faffb4"
)
TERMINAL_VERDICT_SEMANTIC_HASH = (
    "b703064703fda0047afc60637cde6c9a8a32ee1522c39c86be420dfd7727ab49"
)
TERMINAL_VERDICT_TAG = (
    "worm/economic-evolution-account-state-router-0011-verdict-2026-07-14"
)
TERMINAL_VERDICT_COMMIT = "4b3639145db6a1341df92d5443461047734ec01b"
RESULT_SHA256 = "bff23b5e05f7de917a15ab6be496fd8315322ad637484602205c1b6a7b09a1d4"
RESULT_SEMANTIC_HASH = (
    "766c643be49848e18473af236449abf0bba6ac918d1d0590881fa1144746f294"
)
NEXT_CAMPAIGN_ID = "hydra_economic_evolution_account_timeline_0012"
RECEIPT_RELATIVE_PATH = (
    CAMPAIGN_OUTPUT_RELATIVE_PATH / "graveyard_append_receipt.json"
)


class EconomicEvolutionAccountStateTerminalRuntime:
    """Persist the exact 0011 account-state class verdict once."""
    ...
    def advance(self, predecessor: Mapping[str, Any]) -> dict[str, Any]:
        ...
        return account_state_terminal_action(predecessor, verdict, receipt)

    def snapshot(self) -> dict[str, Any]:
        present = any(
            row["mechanism_class"] == "ACCOUNT_STATE_CONDITIONAL_ROLE_ROUTER_V1"
            and row["regime"]
            == (
                "DEVELOPMENT_2023Q3_TO_2024Q3_MULTI_MARKET_"
                "PAST_ONLY_ACCOUNT_STATE_ROUTING"
            )
    ...
    def _verify_predecessor(self, predecessor: Mapping[str, Any]) -> None:
        if (
            ...
            != "ECONOMIC_EVOLUTION_ACCOUNT_STATE_0011_COMPLETE"
            or predecessor.get("economic_account_state_campaign_id") != CAMPAIGN_ID
            or predecessor.get("economic_account_state_campaign_state") != "COMPLETE"
            or predecessor.get("economic_account_state_scientific_status")
            != "ARTEFACT_GEOMETRY_ONLY"
            or predecessor.get("economic_account_state_tripwire_verdict")
            != "ARTEFACT_GEOMETRY_ONLY"
            or int(predecessor.get("economic_account_state_real_policy_count", -1))
            != 512
            or int(
                predecessor.get("economic_account_state_matched_control_count", -1)
            )
            != 512
            or int(
                predecessor.get("economic_account_state_policy_pair_evaluated_count", -1)
            )
            != 512
            or int(
                predecessor.get(
                    "economic_account_state_policies_with_combine_pass_count", -1
                )
            )
            != 0
            ...
            raise EconomicEvolutionRuntimeError(
                "account-state terminal predecessor is not frozen 0011"
            )
    ...
    def _verify_proof_state(self) -> None:
        ...
        if len(reservations) != 1:
            raise EconomicEvolutionRuntimeError(
                "account-state terminal found invalid self reservation count"
            )
    ...
    def _write_or_verify_receipt(
        ...
        receipt = {
            "schema": "hydra_account_state_terminal_graveyard_receipt_v1",
            "campaign_id": CAMPAIGN_ID,
        ...
        if self.receipt_path.is_file():
            ...
            if prior != receipt:
                raise EconomicEvolutionRuntimeError(
                    "account-state terminal receipt drift"
                )
            return prior
...
def load_and_verify_account_state_terminal_verdict(
    root: str | Path,
    *,
    ...
    if _sha256(path) != TERMINAL_VERDICT_SHA256:
        raise EconomicEvolutionRuntimeError("account-state terminal WORM drift")
    ...
    if tagged_commit != TERMINAL_VERDICT_COMMIT:
        raise EconomicEvolutionRuntimeError("account-state terminal tag drift")
    ...
    if hashlib.sha256(tagged_blob).hexdigest() != TERMINAL_VERDICT_SHA256:
        raise EconomicEvolutionRuntimeError(
            "account-state terminal tagged blob drift"
        )
    ...
    if (
        ...
        != float(
            (paired.get("stressed_median_net_delta_usd") or {}).get("median", -2.0)
        )
        ...
        raise EconomicEvolutionRuntimeError(
            "account-state terminal WORM does not match frozen result"
        )
    return verdict


def account_state_terminal_action(
    ...
    return {
        **dict(predecessor),
        ...
        "economic_account_state_terminal_verdict": verdict["terminal_decision"][
            "verdict"
        ],
        "economic_account_state_tombstone_signature_hash": receipt[
            "class_signature_hash"
        ],
        "economic_account_state_graveyard_class_signature_count": int(
            receipt["class_signature_count"]
        ),
        "economic_account_state_graveyard_indexed_object_count": int(
            receipt["indexed_object_count"]
        ),
        "economic_account_state_tombstoned_real_policy_count": int(
        ...
        "reason": (
            "The controller tombstoned only the exact role-conditioned "
            "account-state router. A successor must change account "
            "representation and use new IDs."
        ),
        "principal_blocker": (
            "No 0012 outcome may be generated before account-timeline inputs, "
            "actions, matched chronology controls, thresholds and multiplicity "
            "are WORM."
        ),
    }
...
def _verify_append_result(
    ...
    *,
    ...
    if allow_downstream_growth:
        mismatch = actual_classes < expected_classes or actual_objects < expected_objects
    else:
        mismatch = (
    ...
    if (
        ...
        raise EconomicEvolutionRuntimeError(
            "account-state terminal graveyard append drift"
        )
