...
EXPECTED_GLOBAL_N_TRIALS = 263_684


class V71CrossClockSpeedLeadershipFunnelError(RuntimeError):
    pass


def run_speed_leadership_funnel(
    *,
    ...
    minute, transitions, source_audit = load_speed_leadership_sources(root)
    specs = {row.candidate_id: row for row in candidate_specs(root)}
    signals = generate_signal_population(
        minute, transitions, project_root=root, graveyard_path=None
    )
    ...
    result = {
        "schema": "hydra_v7_1_cross_clock_speed_leadership_funnel_result_v1",
        ...
        "CONTRE": (
            "Relative event-clock duration can remain a generic activity proxy; "
            "positive development evidence cannot advance without the grammar tripwire and power audit."
        ),
        "next_action": "preregister_and_run_speed_leadership_grammar_tripwire",
    }
...
def _verify_inputs(
    ...
    if drift:
        raise V71CrossClockSpeedLeadershipFunnelError(
            "speed-leadership funnel frozen input drift: " + ",".join(drift)
        )
    ...
    if multiplicity_trial_count(proof) < EXPECTED_GLOBAL_N_TRIALS:
        raise V71CrossClockSpeedLeadershipFunnelError(
            "speed-leadership candidate reservation absent"
        )
...
def _verify_signal_manifest(
    ...
    if set(rows) != set(specs) or set(signals) != set(specs):
        raise V71CrossClockSpeedLeadershipFunnelError(
            "speed-leadership candidate set drift"
        )
...
def _write_result(
    ...
    result_path = destination / "v71_cross_clock_speed_leadership_funnel_result.json"
    ...
    report_path = destination / "v71_cross_clock_speed_leadership_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Speed-leadership Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=156 verdict=GREEN",
            f"gate=V71_G5_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=12_structures",
            ...
            "diff_validation=hydra/validation/v71_cross_clock_speed_leadership_funnel.py CONTRE=la_vitesse_relative_peut_etre_un_proxy_d_activite",
...
__all__ = [
    "V71CrossClockSpeedLeadershipFunnelError",
