...
RESERVATION_EVENT_ID = "v7_2_flow_impact_relaxation_reservation_0010"
ANNOTATION_EVENT_ID = (
    "v7_2_flow_impact_relaxation_reservation_0010_commit_annotation"
)
EXPECTED_GLOBAL_N_TRIALS = 264_947
POINT_VALUE = 50.0


class V72FlowImpactRelaxationFunnelError(RuntimeError):
    pass


def run_flow_impact_relaxation_funnel(
    *,
    ...
    minute, states, source_audit = load_flow_impact_sources(root)
    ...
    for candidate_id, spec in sorted(specs.items()):
        ...
        elif not stage0_valid or pooled["event_count"] < int(
    ...
    result = {
        "schema": "hydra_v7_2_flow_impact_relaxation_funnel_result_v1",
        ...
        "campaign_effective_N_trials": 54.0,
        "new_data_purchase_count": 0,
        "protected_holdout_access_count_delta": 0,
        "outbound_order_count": 0,
        "CONTRE": (
            "Post-impulse relaxation is delayed and structurally distinct, but only two "
            "calendar blocks exist; positive development economics cannot advance without "
            "the frozen tripwire, relevant nulls and candidate-specific power."
        ),
        "next_action": "run_preregistered_flow_impact_relaxation_tripwire",
    }
    return _write_result(result, root, Path(output_dir))


def _replay_at_stress(
    spec: V72ImpactCandidateSpec,
    ...
    for signal in signals:
        ...
        if entry is None or exit_row is None:
            raise V72FlowImpactRelaxationFunnelError(
                "flow-impact replay timestamp is absent"
            )
        if entry["contract"] != signal.contract or exit_row["contract"] != signal.contract:
            raise V72FlowImpactRelaxationFunnelError(
                "flow-impact replay explicit-contract drift"
            )
        gross = (
...
def _verify_inputs(
    ...
    if drift:
        raise V72FlowImpactRelaxationFunnelError(
            "flow-impact funnel frozen input drift: " + ",".join(drift)
        )
    ...
    if (
        ...
        != "61a36621de8d0d2d466b03d7071146b740bfcd5f"
    ):
        raise V72FlowImpactRelaxationFunnelError(
            "flow-impact reservation provenance is incomplete"
        )
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V72FlowImpactRelaxationFunnelError("unexpected proof-window state")
    return (
...
def _write_result(
    ...
    result_path = destination / "v72_flow_impact_relaxation_funnel_result.json"
    ...
    report_path = destination / "v72_flow_impact_relaxation_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.2 — Delayed flow-impact Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=190 verdict=GREEN",
            f"gate=V72_G10_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=36_structures",
            ...
            "diff_validation=hydra/validation/v72_flow_impact_relaxation_funnel.py,tests/test_v72_flow_impact_relaxation_funnel.py CONTRE=deux_blocs_et_relaxation_potentiellement_geometrique",
...
__all__ = [
    ...
    "V72FlowImpactRelaxationFunnelError",
