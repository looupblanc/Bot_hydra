...
FREEZE_PATH = "WORM/v7.1-cross-clock-flow-power-audit-0002-2026-07-12.json"
FREEZE_SHA256 = "5fc399bca8a7beab2bcf4972dca99c325f4213f309d28c2427411288b9d92260"
...
FUNNEL_PATH = "reports/v7_1/discovery_0004/v71_cross_clock_flow_funnel_result.json"
FUNNEL_SHA256 = "737e484c7fc51380ccaf588b7dc4bc76f3183530254219a2e621494d025cd828"
TRIPWIRE_PATH = (
    "reports/v7_1/discovery_0004/v71_cross_clock_flow_tripwire_result.json"
)
TRIPWIRE_SHA256 = "95a9ba4d548fb1fad18a0dae2f11d67bfcf50d4d280f14363c1f51d918a3aaa0"
EXPECTED_GLOBAL_N_TRIALS = 263_672


class V71CrossClockFlowPowerAuditError(RuntimeError):
    pass


def run_cross_clock_flow_power_audit(
    *,
    ...
    for candidate in freeze["candidates"]:
        ...
        if spec.specification_hash != candidate["specification_hash"]:
            raise V71CrossClockFlowPowerAuditError("candidate specification drift")
        selected = [
    ...
    result = {
        "schema": "hydra_v7_1_cross_clock_flow_power_audit_result_v1",
        "audit_id": "hydra_v7_1_cross_clock_flow_power_audit_0002",
        "verdict": "GREEN",
        ...
        "tripwire_NULL_RATIO": 0.6363636363636362,
        ...
        "CONTRE": (
            "The two candidates were selected on positive D1 walk-forward results; "
            "power classification remains post-selection evidence and not fresh proof."
        ),
        "prochaine_action": (
            "preregister_and_run_bounded_rolling_combine_for_powered_candidates"
            if powered
            else "queue_independent_confirmation_for_nonfragile_underpowered_candidates"
        ),
    }
...
def _verify_inputs(
    ...
    if drift:
        raise V71CrossClockFlowPowerAuditError(
            "cross-clock power audit frozen input drift: " + ",".join(drift)
        )
    ...
    if calibration.get("verdict") != "GREEN" or calibration.get(
        ...
        raise V71CrossClockFlowPowerAuditError("power calibration is not clean GREEN")
    tripwire = json.loads((root / TRIPWIRE_PATH).read_text(encoding="utf-8"))
    if tripwire.get("verdict") != "GREEN_NULL_ADJUSTED_BASELINE":
        raise V71CrossClockFlowPowerAuditError("cross-clock tripwire is not GREEN")
    ...
    if multiplicity_trial_count(proof) != EXPECTED_GLOBAL_N_TRIALS:
        raise V71CrossClockFlowPowerAuditError("power audit reservation is absent")
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71CrossClockFlowPowerAuditError("unexpected proof-window state")
    ...
    if set(frozen_results) != frozen_ids:
        raise V71CrossClockFlowPowerAuditError("frozen candidate set drift")
    return (
...
def _write_result(
    ...
    result_path = destination / "v71_cross_clock_flow_power_audit_result.json"
    ...
    report_path = destination / "v71_cross_clock_flow_power_audit_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Cross-clock power-aware audit",
            "",
            "[HYDRA-V7] phase=4 step=153 verdict=GREEN",
            f"gate=V71_G4_POWER_AUDIT preuve={displayed}#{result_hash[:8]} tests=2_frozen_candidates",
...
__all__ = [
    "V71CrossClockFlowPowerAuditError",
