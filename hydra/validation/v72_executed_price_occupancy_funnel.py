...
class V72ExecutedPriceOccupancyFunnelError(RuntimeError):
    pass


def run_executed_price_occupancy_funnel(
    *,
    ...
    minute, states, source_audit = load_executed_price_occupancy_sources(root)
    ...
    result = {
        "schema": "hydra_v7_2_executed_price_occupancy_funnel_result_v1",
        ...
        "binding_validation_addendum_path": ADDENDUM_PATH,
        ...
        "CONTRE": (
            "The occupancy class has only 43 sessions across two one-month calendar "
            "blocks; positive development economics cannot advance without the frozen "
            "tripwire and candidate-specific power audit."
        ),
        "next_action": "run_preregistered_executed_price_occupancy_tripwire",
    }
    ...
    *,
    ...
    for signal in signals:
        ...
        if entry is None or exit_row is None:
            raise V72ExecutedPriceOccupancyFunnelError(
                "executed-price occupancy replay timestamp is absent"
            )
        if entry["contract"] != signal.contract or exit_row["contract"] != signal.contract:
            raise V72ExecutedPriceOccupancyFunnelError(
                "executed-price occupancy replay explicit-contract drift"
            )
        gross = (
...
def _verify_inputs(
    ...
    if (
        ...
        or evidence.get("grammar_sha256") != GRAMMAR_SHA256
        or evidence.get("feature_results_seen_before_reservation") is not False
        or evidence.get("signal_results_seen_before_reservation") is not False
        or evidence.get("pnl_results_seen_before_reservation") is not False
    ):
        raise V72ExecutedPriceOccupancyFunnelError(
            "executed-price occupancy reservation provenance is incomplete"
        )
    if "Q4_2024" not in burned_window_ids(proof):
        raise V72ExecutedPriceOccupancyFunnelError("Q4 is not irreversibly BURNED")
    return (
...
def _write_result(
    ...
    result_path = destination / "v72_executed_price_occupancy_funnel_result.json"
    ...
    report_path = destination / "v72_executed_price_occupancy_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.2 — Executed-price occupancy Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=207 verdict=GREEN",
            f"gate=V72_G12_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=24_structures",
            ...
            "diff_validation=hydra/validation/v72_executed_price_occupancy_funnel.py,tests/test_v72_executed_price_occupancy_funnel.py CONTRE=43_sessions_et_deux_blocs",
...
__all__ = [
    "EXPECTED_GLOBAL_N_TRIALS",
    "V72ExecutedPriceOccupancyFunnelError",
    "run_executed_price_occupancy_funnel",
]
