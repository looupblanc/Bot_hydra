...
EXPECTED_GLOBAL_N_TRIALS = 263_886


class V71AggressorRunTopologyFunnelError(RuntimeError):
    pass


def run_aggressor_run_topology_funnel(
    *,
    ...
    minute, states, source_audit = load_aggressor_run_topology_sources(root)
    ...
    all_days = tuple(
        sorted({signal.session_day for rows in signals.values() for signal in rows})
    )
    ...
    for candidate_id, spec in sorted(specs.items()):
        ...
        events = (
            _replay_candidate(
                spec, candidate_signals, replay_cache, cost_model=costs
            )
            if stage0_valid
            else []
        )
        ...
        classification = (
            ...
            if not stage0_valid
            else "FORMULATION_FALSIFIED"
            if not stage1_pass or not walk_positive
            else "WALK_FORWARD_POSITIVE_PENDING_TRIPWIRE_AND_POWER"
        )
        rows.append(
            {
    ...
    result = {
        "schema": "hydra_v7_1_aggressor_run_topology_funnel_result_v1",
        ...
        "CONTRE": "Aggressor-run topology can be contemporaneous minute-price geometry; positive development evidence cannot advance without the frozen tripwire and candidate-specific power audit.",
        "next_action": "run_preregistered_aggressor_run_topology_tripwire",
    }
...
def _verify_inputs(
    ...
    if drift:
        raise V71AggressorRunTopologyFunnelError(
            "aggressor-run funnel frozen input drift: " + ",".join(drift)
        )
    ...
    if multiplicity_trial_count(proof) < EXPECTED_GLOBAL_N_TRIALS:
        raise V71AggressorRunTopologyFunnelError("aggressor-run candidate reservation absent")
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71AggressorRunTopologyFunnelError("unexpected proof-window state")
    return (
        json.loads((root / POLICY_PATH).read_text()),
        json.loads((root / SIGNAL_MANIFEST_PATH).read_text()),
    )
...
def _write_result(result: dict[str, Any], root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    report_path = destination / "v71_aggressor_run_topology_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Aggressor-run topology Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=178 verdict=GREEN",
            f"gate=V71_G9_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=4_structures",
            ...
            "diff_validation=hydra/validation/v71_aggressor_run_topology_funnel.py CONTRE=la_topologie_des_runs_peut_rester_une_geometrie_contemporaine",
...
__all__ = [
    ...
    "V71AggressorRunTopologyFunnelError",
