...
EXPECTED_GLOBAL_N_TRIALS = 263_820


class V71FlowSignSequenceFunnelError(RuntimeError):
    pass


def run_flow_sign_sequence_funnel(
    *,
    ...
    minute, states, source_audit = load_flow_sign_sequence_sources(root)
    ...
    result = {
        "schema": "hydra_v7_1_flow_sign_sequence_funnel_result_v1",
        ...
        "CONTRE": "Sign-sequence states can remain generic combinatorial geometry; positive walk-forward development evidence cannot advance without the frozen tripwire and candidate-specific power audit.",
        "next_action": "run_preregistered_flow_sign_sequence_tripwire",
    }
...
def _verify_inputs(root: Path, proof_registry_path: str | Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if drift:
        raise V71FlowSignSequenceFunnelError("flow-sign funnel frozen input drift: " + ",".join(drift))
    ...
    if multiplicity_trial_count(proof) < EXPECTED_GLOBAL_N_TRIALS:
        raise V71FlowSignSequenceFunnelError("flow-sign candidate reservation absent")
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71FlowSignSequenceFunnelError("unexpected proof-window state")
...
def _write_result(result: dict[str, Any], root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    report_path = destination / "v71_flow_sign_sequence_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Flow-sign sequence Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=169 verdict=GREEN",
            f"gate=V71_G7_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=6_structures",
            ...
            "diff_validation=hydra/validation/v71_flow_sign_sequence_funnel.py CONTRE=les_sequences_peuvent_rester_une_geometrie_combinatoire",
...
__all__ = ["EXPECTED_GLOBAL_N_TRIALS", "V71FlowSignSequenceFunnelError", "run_flow_sign_sequence_funnel"]
