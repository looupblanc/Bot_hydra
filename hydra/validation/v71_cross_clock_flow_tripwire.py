...
TRIPWIRE_POLICY_PATH = "WORM/v7.1-cross-clock-flow-tripwire-0004-2026-07-12.json"
TRIPWIRE_POLICY_SHA256 = (
    "36504626470927c9ee7883c7eac79c7524dee2342cf30ace5277ce1c3629176a"
)
...
SIGNAL_MANIFEST_SHA256 = (
    "35393b8bee755da085b9214fde3a89975221f1f4ac7915251ed3e8f596f758b8"
)
FUNNEL_RESULT_PATH = (
    "reports/v7_1/discovery_0004/v71_cross_clock_flow_funnel_result.json"
)
FUNNEL_RESULT_SHA256 = (
    "737e484c7fc51380ccaf588b7dc4bc76f3183530254219a2e621494d025cd828"
)
EXPECTED_GLOBAL_N_TRIALS = 263_664
NULL_THRESHOLD = 0.80
SHUFFLE_SEEDS = {(2023, "ES"): 950301, (2024, "ES"): 950401}
RANDOM_SEEDS = {(2023, "ES"): 960301, (2024, "ES"): 960401}


class V71CrossClockFlowTripwireError(RuntimeError):
    pass


def run_cross_clock_flow_tripwire(
    *,
    project_root: str | Path = ".",
    proof_registry_path: str | Path = "mission/state/proof_registry.json",
    output_dir: str | Path = "reports/v7_1/discovery_0004",
) -> dict[str, Any]:
    root = Path(project_root).resolve()
    _verify_inputs(root, proof_registry_path)
    minute, real_pairs, source_audit = load_cross_clock_sources(root)
    ...
    real_signals = generate_signal_population(
        minute, real_pairs, project_root=root, graveyard_path=None
    )
    ...
    for control in D1NullControl:
        null_minute, null_event = build_cross_clock_null_world(
            minute, event, control=control
        )
        _verify_preserved_event_observables(event, null_event)
        null_pairs, null_audit = build_cross_clock_pairs(null_minute, null_event)
        if null_audit != source_audit:
            raise V71CrossClockFlowTripwireError(
                "cross-clock null changed source eligibility"
            )
        null_signals = generate_signal_population(
            null_minute, null_pairs, project_root=root, graveyard_path=None
        )
    ...
    if pooled_episodes < int(real["episode_count"]):
        raise V71CrossClockFlowTripwireError("null episode count is below real")
    ...
    result = {
        "schema": "hydra_v7_1_cross_clock_flow_tripwire_result_v1",
        "tripwire_id": "hydra_v7_1_cross_clock_flow_tripwire_0004",
        ...
        "cross_clock_pair_recomputation": True,
        ...
        "CONTRE": (
            "Both event clocks use identical prints and only two D1 calendar "
            "blocks exist; even a green tripwire cannot establish forward persistence."
        ),
        "prochaine_action": (
            "audit_power_of_two_walk_forward_positive_candidates"
            if verdict == "GREEN_NULL_ADJUSTED_BASELINE"
            else "freeze_cross_clock_grammar_as_geometry_contaminated"
        ),
    }
    return _write_result(result, root, Path(output_dir))


def build_cross_clock_null_world(
    minute: pd.DataFrame,
    event: pd.DataFrame,
    *,
...
def _verify_preserved_event_observables(
    ...
    for column in columns:
        if not real_event[column].equals(null_event[column]):
            raise V71CrossClockFlowTripwireError(
                f"cross-clock null changed preserved observable: {column}"
            )
...
def _verify_inputs(root: Path, proof_registry_path: str | Path) -> None:
    ...
    if drift:
        raise V71CrossClockFlowTripwireError(
            "cross-clock tripwire frozen input drift: " + ",".join(drift)
        )
    ...
    if multiplicity_trial_count(proof) != EXPECTED_GLOBAL_N_TRIALS:
        raise V71CrossClockFlowTripwireError(
            "cross-clock tripwire multiplicity reservation is absent"
        )
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71CrossClockFlowTripwireError("unexpected proof-window state")
...
def _verify_signal_manifest(
    ...
    if set(rows) != set(specs) or set(signals) != set(specs):
        raise V71CrossClockFlowTripwireError("cross-clock signal manifest drift")
    if manifest.get("contains_outcomes_or_pnl") is not False:
        raise V71CrossClockFlowTripwireError("cross-clock manifest contains outcomes")
    for candidate_id, spec in specs.items():
        if rows[candidate_id]["specification_hash"] != spec.specification_hash:
            raise V71CrossClockFlowTripwireError("cross-clock specification drift")
        if rows[candidate_id]["signal_path_hash"] != signal_path_hash(
            signals[candidate_id]
        ):
            raise V71CrossClockFlowTripwireError("cross-clock signal path drift")
...
def _write_result(
    ...
    result_path = destination / "v71_cross_clock_flow_tripwire_result.json"
    ...
    report_path = destination / "v71_cross_clock_flow_tripwire_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Cross-clock flow grammar tripwire",
            "",
            f"[HYDRA-V7] phase=4 step=152 verdict={result['verdict']}",
            f"gate=V71_G4_TRIPWIRE preuve={displayed}#{result_hash[:8]} tests=12_real_plus_36_null_paths",
...
__all__ = [
    "V71CrossClockFlowTripwireError",
    "build_cross_clock_null_world",
    "run_cross_clock_flow_tripwire",
]
