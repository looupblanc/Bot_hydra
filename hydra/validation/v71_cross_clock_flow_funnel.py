...
SIGNAL_MANIFEST_PATH = "reports/v7_1/discovery_0004/v71_cross_clock_flow_signal_manifest.json"
...
EXPECTED_GLOBAL_N_TRIALS = 263_616


class V71CrossClockFlowFunnelError(RuntimeError):
    pass


def run_cross_clock_flow_funnel(
    *,
    ...
    policy, power_policy, manifest = _verify_inputs(root, proof_registry_path)
    minute, pairs, source_audit = load_cross_clock_sources(root)
    specs = {row.candidate_id: row for row in candidate_specs(root)}
    signals = generate_signal_population(minute, pairs, project_root=root)
    ...
    for candidate_id, spec in sorted(specs.items()):
        ...
        if duplicate_of:
            ...
            classification = "WALK_FORWARD_POSITIVE_PENDING_POWER_AUDIT"
        rows.append(
            {
    ...
    result = {
        "schema": "hydra_v7_1_cross_clock_flow_funnel_result_v1",
        ...
        "CONTRE": "Cross-clock states can duplicate the same print information; a positive development funnel cannot advance without the permanent grammar tripwire and candidate-specific power audit.",
        "next_action": "preregister_and_run_cross_clock_grammar_tripwire",
    }
...
def _verify_inputs(
    ...
    if drift:
        raise V71CrossClockFlowFunnelError(
            "cross-clock funnel frozen input drift: " + ",".join(drift)
        )
    ...
    if multiplicity_trial_count(proof) < EXPECTED_GLOBAL_N_TRIALS:
        raise V71CrossClockFlowFunnelError("cross-clock candidate reservation absent")
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71CrossClockFlowFunnelError("unexpected proof-window state")
    return (
        json.loads((root / POLICY_PATH).read_text(encoding="utf-8")),
        json.loads((root / POWER_POLICY_PATH).read_text(encoding="utf-8")),
...
def _write_result(result: dict[str, Any], root: Path, output_dir: Path) -> dict[str, Any]:
    ...
    report_path = destination / "v71_cross_clock_flow_funnel_report.md"
    try:
        evidence_path = result_path.relative_to(root)
    except ValueError:
        evidence_path = result_path
    report = "\n".join(
        [
            "# HYDRA V7.1 — Limited cross-clock Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=151 verdict=GREEN",
            f"gate=V71_G4_STAGE0_STAGE2 preuve={evidence_path}#{result_hash[:8]} tests=12_structures",
            ...
            "diff_validation=hydra/validation/v71_cross_clock_flow_funnel.py CONTRE=les_deux_horloges_peuvent_dupliquer_les_memes_prints",
...
__all__ = ["V71CrossClockFlowFunnelError", "run_cross_clock_flow_funnel"]
