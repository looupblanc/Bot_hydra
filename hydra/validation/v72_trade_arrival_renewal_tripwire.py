...
TRIPWIRE_POLICY_SHA256 = "1e42cb7fcb70542096b0ddab559ade74bf1e9cb4cfd49150e643a804d41db4dc"
...
NULL_THRESHOLD = 0.80
SHUFFLE_SEEDS = {(2023, "ES"): 7211001, (2024, "ES"): 7211001}
RANDOM_SEEDS = {(2023, "ES"): 7211002, (2024, "ES"): 7211002}
...
class V72TradeArrivalRenewalTripwireError(RuntimeError):
    pass


def run_trade_arrival_renewal_tripwire(
    *,
    ...
    minute, real_states, source_audit = load_trade_arrival_renewal_sources(root)
    ...
    for control in D1NullControl:
        null_minute, null_feature = build_trade_arrival_null_world(
            minute, feature, control=control
        )
        _verify_preserved_observables(minute, feature, null_minute, null_feature)
        null_states, null_audit = build_trade_arrival_renewal_states(
    ...
    if int(real["episode_count"]) != 480 or pooled_episodes != 1440:
        raise V72TradeArrivalRenewalTripwireError(
            "trade-arrival tripwire denominator drift"
        )
    ...
    result = {
        "schema": "hydra_v7_2_trade_arrival_renewal_tripwire_result_v1",
        "tripwire_id": "hydra_v7_2_trade_arrival_renewal_tripwire_0011",
        ...
        "arrival_and_timestamp_observables_preserved_in_price_nulls": True,
        "price_progress_signals_and_account_paths_recomputed": True,
        ...
        "CONTRE": (
            "Arrival timestamps are intentionally invariant in the price nulls, so "
            "time-of-day seasonality can remain; raw counts and exact-binomial evidence "
            "must be read separately from candidate economics."
        ),
        "prochaine_action": (
            "freeze_power_audit_for_2x_surviving_walk_forward_candidates"
            if verdict == "GREEN_NULL_ADJUSTED_BASELINE"
            else "tombstone_trade_arrival_renewal_class_as_geometry_only"
...
def build_trade_arrival_null_world(
    minute: pd.DataFrame,
    ...
    *,
    ...
    if control == D1NullControl.YEAR_BLOCK_PERMUTATION:
        null_minute = _year_permuted_prices(minute, "minute_start_ns")
    else:
        seeds = (
            SHUFFLE_SEEDS
        ...
        null_minute = _within_year_price_null(
            minute, "minute_start_ns", control=control, seeds=seeds
        )
    keys = ["calendar_year", "contract", "minute_start_ns", "availability_ns"]
    if not feature[keys].reset_index(drop=True).equals(
        null_minute[keys].reset_index(drop=True)
    ):
        raise V72TradeArrivalRenewalTripwireError(
            "trade-arrival null identity alignment failed"
        )
    null_feature = feature.copy()
    null_feature["first_price"] = null_minute["open"].to_numpy(float)
    null_feature["last_price"] = null_minute["close"].to_numpy(float)
    null_feature["price_progress_points"] = null_minute[
        "price_change_points"
    ].to_numpy(float)
...
def _verify_preserved_observables(
    ...
    for column in INVARIANT_FEATURE_COLUMNS:
        if not real_feature[column].equals(null_feature[column]):
            raise V72TradeArrivalRenewalTripwireError(
                f"trade-arrival null changed invariant feature: {column}"
            )
    for column in ("minute_start_ns", "availability_ns", "calendar_year", "contract"):
        if not real_minute[column].equals(null_minute[column]):
            raise V72TradeArrivalRenewalTripwireError(
                f"trade-arrival null changed minute identity: {column}"
            )
...
def _verify_invariant_source_counts(
    ...
    for field in ("minute_count", "session_count", "exact_source_match_count"):
        if int(real[field]) != int(null[field]):
            raise V72TradeArrivalRenewalTripwireError(
                f"trade-arrival null source-count drift: {field}"
            )
...
def _verify_inputs(root: Path, proof_registry_path: str | Path) -> None:
    ...
    if (
        ...
        raise V72TradeArrivalRenewalTripwireError(
            "trade-arrival tripwire reservation provenance is incomplete"
        )
    if "Q4_2024" not in burned_window_ids(proof):
        raise V72TradeArrivalRenewalTripwireError("Q4 is not irreversibly BURNED")
...
def _write_result(
    ...
    result_path = destination / "v72_trade_arrival_renewal_tripwire_result.json"
    ...
    report_path = destination / "v72_trade_arrival_renewal_tripwire_report.md"
    ...
    report = "\n".join(
        [
            ...
            "",
            f"[HYDRA-V7] phase=4 step=200 verdict={result['verdict']}",
            f"gate=V72_G11_TRIPWIRE preuve={displayed}#{result_hash[:8]} tests=real_vs_3_null_worlds",
...
__all__ = [
    "EXPECTED_GLOBAL_N_TRIALS",
    "V72TradeArrivalRenewalTripwireError",
