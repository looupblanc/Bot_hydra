...
TRIPWIRE_POLICY_PATH = (
    "WORM/v7.1-trade-size-composition-tripwire-0006-2026-07-13.json"
)
TRIPWIRE_POLICY_SHA256 = (
    "c10f9b671cd6ddb0349de05673c5854e3b37776001224a40b218c0a39b39dea9"
)
...
EXPECTED_GLOBAL_N_TRIALS = 263_762
NULL_THRESHOLD = 0.80
SHUFFLE_SEEDS = {(2023, "ES"): 990301, (2024, "ES"): 990401}
RANDOM_SEEDS = {(2023, "ES"): 991301, (2024, "ES"): 991401}


class V71TradeSizeCompositionTripwireError(RuntimeError):
    pass


def run_trade_size_composition_tripwire(
    *,
    ...
    real_states, source_audit = build_trade_size_composition_states(minute)
    ...
    for control in D1NullControl:
        null_minute = build_trade_size_composition_null_world(
            minute, control=control
        )
        _verify_preserved_trade_observables(minute, null_minute)
        null_states, null_audit = build_trade_size_composition_states(null_minute)
        ...
        summary["source_audit"] = null_audit.to_dict()
    ...
    if pooled_episodes < int(real["episode_count"]):
        raise V71TradeSizeCompositionTripwireError(
    ...
    result = {
        "schema": "hydra_v7_1_trade_size_composition_tripwire_result_v1",
        "tripwire_id": "hydra_v7_1_trade_size_composition_tripwire_0006",
        ...
        "trade_composition_recomputed_in_each_null": True,
        "price_derived_fields_recomputed_in_each_null": True,
        "trade_count_volume_and_flow_preserved_in_price_nulls": True,
        ...
        "CONTRE": (
            "Only two D1 calendar blocks exist and trade composition is held fixed "
            "in price nulls; even a green result remains development evidence requiring power audit and fresh confirmation."
        ),
        "prochaine_action": (
            "audit_power_of_trade_size_walk_forward_positive_candidates"
            if verdict == "GREEN_NULL_ADJUSTED_BASELINE"
            else "freeze_trade_size_composition_class_as_geometry_contaminated"
        ),
    }
    return _write_result(result, root, Path(output_dir))


def build_trade_size_composition_null_world(
    minute: pd.DataFrame,
    *,
...
def _verify_preserved_trade_observables(
    real: pd.DataFrame, null: pd.DataFrame
) -> None:
    columns = (
        "minute_start_ns",
        "source_close_ns",
    ...
    for column in columns:
        if not real[column].equals(null[column]):
            raise V71TradeSizeCompositionTripwireError(
                f"trade-size null changed preserved observable: {column}"
            )


def _verify_invariant_source_counts(
    real: Mapping[str, int], null: Mapping[str, int]
) -> None:
    for field in (
        "minute_count",
        "session_count",
        "same_contract_prior_session_baseline_count",
        "executable_state_count_30m",
        "executable_state_count_60m",
    ):
        if int(real[field]) != int(null[field]):
            raise V71TradeSizeCompositionTripwireError(
                f"trade-size null changed invariant eligibility: {field}"
            )
...
def _verify_inputs(root: Path, proof_registry_path: str | Path) -> None:
    ...
    if drift:
        raise V71TradeSizeCompositionTripwireError(
            "trade-size composition tripwire frozen input drift: " + ",".join(drift)
        )
    ...
    if multiplicity_trial_count(proof) < EXPECTED_GLOBAL_N_TRIALS:
        raise V71TradeSizeCompositionTripwireError(
            "trade-size composition tripwire reservation is absent"
        )
...
def _verify_signal_manifest(
    ...
    if set(rows) != set(specs) or set(signals) != set(specs):
        raise V71TradeSizeCompositionTripwireError(
            "trade-size composition signal manifest drift"
        )
...
def _write_result(
    ...
    result_path = destination / "v71_trade_size_composition_tripwire_result.json"
    ...
    report_path = destination / "v71_trade_size_composition_tripwire_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Trade-size composition permanent tripwire",
            "",
            f"[HYDRA-V7] phase=4 step=162 verdict={result['verdict']}",
            f"gate=V71_G6_TRIPWIRE preuve={displayed}#{result_hash[:8]} tests=real_vs_3_null_worlds",
...
__all__ = [
    ...
    "V71TradeSizeCompositionTripwireError",
    "build_trade_size_composition_null_world",
    "run_trade_size_composition_tripwire",
]
