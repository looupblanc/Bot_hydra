...
from hydra.validation.v71_opportunity_density_funnel import _aggregate
...
SIGNAL_MANIFEST_SHA256 = "e515a0ab84600edfd8552c46b3471f77d0ba17ad3b761cf7757d5fdaa89c736d"
...
EXPECTED_GLOBAL_N_TRIALS = 262_996
FROZEN_RESULT_PATH = "reports/v7_1/discovery_0003/v71_event_time_funnel_result.json"
FROZEN_RESULT_SHA256 = (
    "22f9816aeb2bae8734571dcd84485f0ccbfdb21b4735cbe0ed11356dcbc0358b"
)


class V71EventTimeFunnelError(RuntimeError):
    pass


def run_event_time_funnel(
    *,
    ...
    signals = generate_signal_population(minute, event, project_root=root)
    ...
    result.update(
        {
            "schema": "hydra_v7_1_event_time_funnel_result_v1",
            ...
            "CONTRE": (
                "Event-time states can be deterministic session-liquidity "
                "clocks without directional expectancy; powered positive "
                "walk-forward transfer and the permanent tripwire remain mandatory."
            ),
            "next_action": (
                "run_event_time_new_grammar_tripwire_before_candidate_nulls"
                if result["powered_walk_forward_candidate_count"] > 0
                else "classify_event_time_grammar_and_select_next_information_action"
            ),
        }
    )
...
def _verify_inputs(
    ...
    if drift:
        raise V71EventTimeFunnelError(
            "event-time frozen input drift: " + ",".join(drift)
        )
    ...
    if burned_window_ids(proof) != ("Q4_2024",):
        raise V71EventTimeFunnelError("unexpected proof-window state")
    return (
...
def _verify_historical_multiplicity_checkpoint(
    ...
    if EXPECTED_GLOBAL_N_TRIALS not in checkpoints or current < EXPECTED_GLOBAL_N_TRIALS:
        raise V71EventTimeFunnelError("event-time candidate reservation is absent")
    if (
        current != EXPECTED_GLOBAL_N_TRIALS
        and _sha256(root / FROZEN_RESULT_PATH) != FROZEN_RESULT_SHA256
    ):
        raise V71EventTimeFunnelError(
            "event-time multiplicity suffix requires frozen historical result"
        )
...
def _write_result(
    ...
    result_path = destination / "v71_event_time_funnel_result.json"
    ...
    report_path = destination / "v71_event_time_funnel_report.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Event-time Stage 0–2",
            "",
            "[HYDRA-V7] phase=4 step=141 verdict=GREEN",
            f"gate=V71_G3_STAGE0_STAGE2 preuve={displayed}#{result_hash[:8]} tests=128_structures",
...
__all__ = ["V71EventTimeFunnelError", "run_event_time_funnel"]
