...
ACCOUNT_TIMELINE_ENGINE_VERSION = "hydra_account_timeline_campaign_v1"


class AccountTimelineCampaignError(RuntimeError):
    pass


def run_account_timeline_campaign(
    output_dir: str | Path,
    *,
    ...
    """Evaluate frozen own-sleeve outcome timelines against deranged controls."""

    started = time.perf_counter()
    phases: dict[str, float] = {}
    phase_started = started
    prereg_path = Path(preregistration_path).resolve()
    prereg = load_and_verify_account_timeline_preregistration(prereg_path)
    ...
    if _sha256(seed_path) != str(prereg["source_seed"]["file_sha256"]):
        raise AccountTimelineCampaignError("source seed checksum drift")
    if seed["archive_hash"] != str(prereg["source_seed"]["archive_hash"]):
        raise AccountTimelineCampaignError("source seed semantic hash drift")
    ...
    if population.manifest_hash != expected_manifest:
        raise AccountTimelineCampaignError("frozen account-timeline manifest drift")
    writer.write_json(
        "account_timeline_population.json",
        {
            **population.summary(),
    ...
    _stage(state_writer, prereg, "ACCOUNT_TIMELINE_POPULATION_FROZEN")
    ...
    if len(screen.rows) != len(sleeves):
        raise AccountTimelineCampaignError("component cheap screen is incomplete")
    ...
    if len(runtimes) != len(sleeves) or exact_failures:
        raise AccountTimelineCampaignError(
            "account-timeline replay requires every frozen component runtime"
        )
    ...
    if len(starts) != int(prereg["rolling_episode_policy"]["maximum_starts"]):
        raise AccountTimelineCampaignError(
            "account-timeline campaign has incomplete frozen episode starts"
        )

    phase_started = time.perf_counter()
    pair_rows = evaluate_account_timeline_policy_pairs(
        population.pairs,
        runtimes,
        starts=starts,
    ...
    if len(pair_rows) != expected_pairs:
        raise AccountTimelineCampaignError(
            "account-timeline paired evaluation is incomplete"
        )
    writer.write_jsonl_batch("account_timeline_pair_results.jsonl", pair_rows)
    ...
    writer.write_json("account_timeline_result.json", result)
    writer.write_text(
        "account_timeline_report.md", account_timeline_report(result, prereg)
    )
    _stage(state_writer, prereg, "COMPLETE")
    return result


def load_and_verify_account_timeline_preregistration(
    path: str | Path,
) -> dict[str, Any]:
    ...
    if (
        prereg.get("schema") != "hydra_account_timeline_preregistration_v1"
        or prereg.get("class_id") != ACCOUNT_TIMELINE_CLASS_ID
        ...
        or structural.get("same_completed_history_values_within_pair") is not True
        or structural.get("same_history_source_multiset_within_pair") is not True
        or structural.get("same_timeline_thresholds_within_pair") is not True
        or structural.get("past_only_completed_outcomes") is not True
        or structural.get("new_market_outcomes_seen_during_generation") is not False
        or structural.get("same_class_0011_rescue") is not False
        or prereg.get("account_timeline_policy") != ACCOUNT_TIMELINE_LIMITS
        or float(
            tripwire.get(
        ...
        raise AccountTimelineCampaignError(
            "invalid account-timeline preregistration"
        )
    ...
    for relative, expected in prereg.get("implementation_files", {}).items():
        if _sha256(root / str(relative)) != str(expected):
            raise AccountTimelineCampaignError(
                f"account-timeline implementation drift: {relative}"
            )
    return prereg


def load_and_verify_account_timeline_result(
    ...
    if (
        ...
        or result.get("class_id") != ACCOUNT_TIMELINE_CLASS_ID
        ...
        raise AccountTimelineCampaignError(
            "account-timeline result integrity failure"
        )
    return result


def account_timeline_paired_tripwire(
...
def account_timeline_final_result(
    ...
    *,
    ...
    return {
        "schema": "hydra_account_timeline_result_v1",
        "engine_version": ACCOUNT_TIMELINE_ENGINE_VERSION,
        ...
        "next_action": (
            "SCALE_ACCOUNT_TIMELINE_SURVIVORS"
            if combine or research
            else "TOMBSTONE_EXACT_0012_AND_CHANGE_ACCOUNT_REPRESENTATION"
        ),
        "CONTRE": (
            "Completed sleeve outcomes can be persistent for purely mechanical "
            "reasons such as clustered opportunities or common market beta. "
            "Only uplift over the within-membership history-source permutation "
            "with unchanged MLL and consistency supports this exact class."
        ),
    }


def _account_decision(
    row: Mapping[str, Any], *, gate: Mapping[str, Any], family_green: bool
) -> dict[str, Any]:
    value = dict(row)
    ...
    value.update(
        {
            "status": (
                ...
                if research
                else "ACCOUNT_TIMELINE_DIAGNOSTIC_ONLY"
                if diagnostic
                else "ACCOUNT_TIMELINE_RESEARCH_FAILED"
            ),
...
def _targeted_mutations(failures: Mapping[str, int]) -> list[dict[str, Any]]:
    actions = {
        "TARGET_VELOCITY_LOW": "CHANGE_ACCOUNT_REPRESENTATION_NOT_TIMELINE_THRESHOLDS",
        "NO_COMBINE_PASS": "ADD_STRUCTURALLY_DISTINCT_ACCOUNT_MECHANISM",
        "MLL_BREACH_EXCESS": "ADD_CAUSAL_BUFFER_DERISK_NEW_CLASS_ONLY",
        "CONSISTENCY_FAILURE": "ADD_CAUSAL_PROFIT_SMOOTHER_NEW_CLASS_ONLY",
        "STRESSED_ECONOMICS_NONPOSITIVE": "REMOVE_COSTLY_TIMELINE_ACTION",
        "TEMPORAL_BLOCK_INSTABILITY": "REPLACE_TIMELINE_MECHANISM_NEW_CLASS_ONLY",
        "COMPONENT_CONCENTRATION": "ADD_DISTINCT_CAUSAL_DIVERSIFIER",
        "MATCHED_CONTROL_NOT_BEATEN": "TOMBSTONE_ACCOUNT_TIMELINE_CLASS",
    }
...
def account_timeline_report(
    ...
    tests = prereg["reporting"]["predeployment_regression_floor"]
    evidence_path = "reports/economic_evolution/account_timeline_0012/account_timeline_result.json"
    lines = [
        f"[HYDRA-V7] phase=4 step=0012 verdict={result['report_verdict']}",
        f"gate=ACCOUNT_TIMELINE_TRIPWIRE preuve={evidence_path}#PENDING_WRITE tests={tests}/{tests}",
        f"budget_llm=0/{prereg['budget']['llm_phase_max_usd']} budget_data={budget['actual_spend_usd']}/{budget['hard_cap_usd']} N_trials={prereg['multiplicity']['expected_global_N_trials_after_reservation']} burned={prereg['reporting']['burned_window_count']}",
        "diff_validation=hydra/economic_evolution/account_timeline_evaluation.py",
        f"CONTRE={result['CONTRE']}",
        f"prochaine_action={result['next_action']}",
        "",
        "# HYDRA economic evolution — account timeline 0012",
        "",
        f"- Verdict: `{result['scientific_status']}` / `{result['report_verdict']}`",
        f"- Policies: {result['policy_pair_evaluated_count']} real + history-permutation controls",
...
def _verify_frozen_json_reference(
    ...
    if _sha256(path) != str(reference["file_sha256"]):
        raise AccountTimelineCampaignError(f"frozen JSON checksum drift: {path}")
    value = _load_json(path)
    if value.get(semantic_key) != reference["semantic_hash"]:
        raise AccountTimelineCampaignError(f"frozen JSON semantic drift: {path}")
...
def _stage(
    ...
    writer.write_json(
        "account_timeline_campaign_state.json",
        {
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise AccountTimelineCampaignError(f"invalid JSON: {path}") from exc
    if not isinstance(value, dict):
        raise AccountTimelineCampaignError(f"expected JSON object: {path}")
    return value
...
def _project_root(path: Path) -> Path:
    for parent in (path.parent, *path.parents):
        if (parent / "MISSION_CONTRACT.md").is_file():
            return parent
    raise AccountTimelineCampaignError("project root not found")
