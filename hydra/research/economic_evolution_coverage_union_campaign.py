...
COVERAGE_UNION_ENGINE_VERSION = "hydra_coverage_union_campaign_v1"


class CoverageUnionCampaignError(RuntimeError):
    pass


def run_coverage_union_campaign(
    output_dir: str | Path,
    *,
    ...
    prereg = load_and_verify_coverage_union_preregistration(prereg_path)
    ...
    if _sha256(seed_path) != str(prereg["source_seed"]["file_sha256"]):
        raise CoverageUnionCampaignError("coverage-union seed checksum drift")
    if seed["archive_hash"] != str(prereg["source_seed"]["archive_hash"]):
        raise CoverageUnionCampaignError("coverage-union seed semantic drift")

    structural = prereg["structural_population"]
    population = generate_coverage_union_population(
        seed,
    ...
    if population.manifest_hash != str(structural["policy_manifest_hash"]):
        raise CoverageUnionCampaignError("frozen coverage-union manifest drift")
    writer.write_json(
        "coverage_union_population.json",
        {
            **population.summary(),
    ...
    _stage(state_writer, prereg, "COVERAGE_UNION_POPULATION_FROZEN")
    ...
    if len(screen.rows) != len(sleeves):
        raise CoverageUnionCampaignError("coverage component screen is incomplete")
    ...
    if len(runtimes) != len(sleeves) or exact_failures:
        raise CoverageUnionCampaignError(
            "coverage union requires every frozen component runtime"
        )
    ...
    if len(starts) != int(prereg["rolling_episode_policy"]["maximum_starts"]):
        raise CoverageUnionCampaignError("incomplete frozen coverage starts")

    phase_started = time.perf_counter()
    pair_rows = evaluate_coverage_union_policy_pairs(
        population.pairs,
        runtimes,
        starts=starts,
    ...
    if len(pair_rows) != expected_pairs:
        raise CoverageUnionCampaignError("coverage paired evaluation is incomplete")
    writer.write_jsonl_batch("coverage_union_pair_results.jsonl", pair_rows)
    ...
    tripwire["class_id"] = COVERAGE_UNION_CLASS_ID
    ...
    result.update(
        {
            "schema": "hydra_coverage_union_result_v1",
            "engine_version": COVERAGE_UNION_ENGINE_VERSION,
            "class_id": COVERAGE_UNION_CLASS_ID,
            "next_action": (
                "SCALE_AND_MUTATE_COVERAGE_UNION_SURVIVORS"
                if int(result["combine_path_diagnostic_count"])
                or int(result["account_research_candidate_count"])
                else "TOMBSTONE_EXACT_0014_AND_CHANGE_REPRESENTATION"
            ),
    ...
    for row in result["best_development_policies"]:
        row["status"] = str(row["status"]).replace(
            "ACCOUNT_TIMELINE", "COVERAGE_UNION"
        )
    ...
    writer.write_json("coverage_union_result.json", result)
    writer.write_text("coverage_union_report.md", _report(result, prereg))
    _stage(state_writer, prereg, "COMPLETE")
    return result


def load_and_verify_coverage_union_preregistration(
    path: str | Path,
) -> dict[str, Any]:
    ...
    if (
        ...
        or prereg.get("class_id") != COVERAGE_UNION_CLASS_ID
        or stable_hash(payload) != claimed
        or int(structural.get("policy_pair_count", -1)) != 512
        or int(structural.get("component_count", -1)) != 48
        or structural.get("union_sizes") != [10, 11, 12]
        or structural.get("minimum_real_markets") != 4
        or structural.get("minimum_real_sessions") != 4
        or structural.get("maximum_control_markets") != 2
        or float(structural.get("maximum_event_count_relative_delta", -1.0))
        != 0.15
        or prereg.get("coverage_union_policy") != COVERAGE_UNION_LIMITS
        ...
        raise CoverageUnionCampaignError("invalid coverage-union preregistration")
    ...
    for relative, expected in prereg.get("implementation_files", {}).items():
        if _sha256(root / str(relative)) != str(expected):
            raise CoverageUnionCampaignError(
                f"coverage-union implementation drift: {relative}"
            )
    return prereg


def load_and_verify_coverage_union_result(
    path: str | Path,
    ...
    if (
        result.get("schema") != "hydra_coverage_union_result_v1"
        or result.get("campaign_id") != prereg.get("campaign_id")
        or result.get("class_id") != COVERAGE_UNION_CLASS_ID
        ...
        raise CoverageUnionCampaignError("coverage-union result integrity failure")
    return result


def _targeted_mutations(failures: Mapping[str, int]) -> list[dict[str, Any]]:
    actions = {
        "NO_COMBINE_PASS": "CHANGE_TARGET_VELOCITY_REPRESENTATION_NEW_CLASS",
        "TARGET_VELOCITY_LOW": "ADD_CAUSAL_EXIT_OR_OPPORTUNITY_MECHANISM",
        "MLL_BREACH_EXCESS": "REDUCE_CORRELATED_COVERAGE_CELLS",
        "CONSISTENCY_FAILURE": "ADD_CAUSAL_ACCOUNT_PROFIT_SMOOTHER",
        "MATCHED_CONTROL_NOT_BEATEN": "TOMBSTONE_COVERAGE_UNION_CLASS",
        "STRESSED_ECONOMICS_NONPOSITIVE": "REMOVE_COSTLY_COVERAGE_CELL",
        "TEMPORAL_BLOCK_INSTABILITY": "CHANGE_COVERAGE_REPRESENTATION",
        "COMPONENT_CONCENTRATION": "REMOVE_DOMINANT_COVERAGE_COMPONENT",
    }
...
def _report(result: Mapping[str, Any], prereg: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# HYDRA coverage-union campaign 0014",
        "",
...
def _verify_frozen_json_reference(
    ...
    *,
    ...
    if _sha256(path) != str(reference["file_sha256"]):
        raise CoverageUnionCampaignError("coverage WORM checksum drift")
    if _load_json(path).get(semantic_key) != reference["semantic_hash"]:
        raise CoverageUnionCampaignError("coverage WORM semantic drift")
...
def _stage(
    ...
    writer.write_json(
        "coverage_union_campaign_state.json",
        {
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CoverageUnionCampaignError(f"invalid JSON: {path}") from exc
    if not isinstance(value, dict):
        raise CoverageUnionCampaignError(f"expected JSON object: {path}")
    return value
...
__all__ = [
    "COVERAGE_UNION_ENGINE_VERSION",
    "CoverageUnionCampaignError",
    "load_and_verify_coverage_union_preregistration",
    "load_and_verify_coverage_union_result",
