...
def _priority_index(basket: BasketPolicy, component_id: str) -> int:
    """Return the frozen sleeve-priority index used for deterministic replay."""
...
def run_shared_account_episode(
    ...
    *,
    ...
    active_pool_policy: ActiveRiskPoolPolicy | None = None,
    config: Topstep150KConfig | None = None,
) -> AccountPolicyEpisode:
    if controller is not None and active_pool_policy is not None:
        raise ValueError("shared account replay accepts only one account governor")
    ...
    for elapsed, day in enumerate(episode_days, start=1):
        ...
        if active_pool_policy is not None or basket.policy_version.startswith(
        ...
        for timestamp, kind, _priority, event_id, trade in actions:
            ...
            decision = (
                route_active_risk_entry(intent, state, policy=active_pool_policy)
                if active_pool_policy is not None
                else route_entry(intent, state, policy=controller)
            ...
            if active_pool_policy is not None:
                # These values are diagnostic attribution recorded only after
                # the causal routing decision is frozen.  The router never
                # receives the realized trade outcome, and no pre-outcome
                # expected-PnL model exists in the immutable source bundle.
                requested = int(
                    decision_row.get("requested_quantity", event.quantity)
                )
                ...
                decision_row.update(
                    {
                        ...
                        "foregone_realized_pnl_ex_post": float(
                            event.net_pnl * suppressed / max(event.quantity, 1)
                        ),
    ...
    return AccountPolicyEpisode(
        policy_id=(
            active_pool_policy.policy_id
            if active_pool_policy is not None
            else controller.controller_id
            if controller is not None
            else basket.policy_id
        ),
