...
def generate_candidates(count: int, symbols: list[str], timeframes: list[str], seed: int) -> list[StrategyCandidate]:
    ...
    for _ in range(count):
        ...
        params = _params_for_family(family, rng)
        risk = {
            "holding_period": int(rng.integers(3, 18)),
            "risk_scale": float(rng.uniform(0.45, 1.25)),
            "max_position": 1,
        }
        candidates.append(
            StrategyCandidate(
...
def _params_for_family(family: str, rng: np.random.Generator) -> dict[str, float]:
