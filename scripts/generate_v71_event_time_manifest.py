...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free V7.1 event-time manifest."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output-dir", default="reports/v7_1/discovery_0003")
    ...
    signals = generate_signal_population(
        minute, event, project_root=root
    )
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_event_time_signal_manifest_v1",
        ...
        "CONTRE": (
            "Event completion speed can be a session-liquidity clock rather "
            "than directional edge; economic replay and a new-grammar tripwire "
            "remain mandatory."
        ),
    }
    ...
    result_path = output / "v71_event_time_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    report_path = output / "v71_event_time_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Event-time outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=140 verdict=GREEN",
            f"gate=V71_G3_SIGNAL_FREEZE preuve={result_path.relative_to(root)}#{result_hash[:8]} tests=outcome_free_generation",
            ...
            f"- Barres cross-date exclues: `{audit.cross_chicago_date_count}`",
            "",
            "## CONTRE",
            "",
            str(payload["CONTRE"]),
