#!/usr/bin/env python3
"""Governed one-shot ZC/ZS/ZW acquisition for the USDA grain tripwire."""
...
MANIFEST = Path("config/research/usda_grain_information_shock_tripwire_v1.json")
RECEIPT = Path("reports/data_access/usda_grain_information_shock_acquisition_receipt.json")
LOCAL_LOCK = Path("reports/data_access/usda_grain_information_shock_acquisition.lock")
GLOBAL_LOCK = Path("reports/data_access/fresh_confirmation_0035_acquisition.lock")
CACHE_ROOT = Path("data/cache/databento/usda_grain_information_shock")
ACCESS_LEDGER = Path("reports/data_access/data_access_ledger.jsonl")
PURPOSE = (
    "bounded pre-Q4-2024 official-USDA WASDE ZC/ZS/ZW development tripwire; "
    "chronological roles frozen before acquisition; no broker, orders, XFA or promotion"
)


class USDAGrainAcquisitionError(RuntimeError):
    pass
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if stable_hash(core) != claimed:
        raise USDAGrainAcquisitionError("frozen manifest hash drift")
    contract = manifest["data_contract"]
    if (
        contract["symbols"] != ["ZC.c.0", "ZS.c.0", "ZW.c.0"]
        or contract["start"] != "2018-01-02"
        or contract["end_exclusive"] != "2024-10-01"
        or contract.get("q4_2024_access") is not False
        or manifest["governance"].get("maximum_cpu_workers") != 1
        or len(manifest["release_contract"]["release_dates"]) != 80
    ):
        raise USDAGrainAcquisitionError("data, release or worker contract drift")
    for item in manifest["frozen_inputs"].values():
        path = root / item["path"]
        if not path.is_file() or sha256_file(path) != item["sha256"]:
            raise USDAGrainAcquisitionError(f"frozen input drift: {path}")
    return manifest
...
def _live_estimates(client: Any, manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for schema, request in _requests(manifest).items():
        ...
        if (
            ...
            raise USDAGrainAcquisitionError(f"official estimate drift: {schema}")
        output[schema] = row
    return output
...
def _append_access_once(root: Path, manifest: dict[str, Any], bundle_id: str) -> None:
    ...
    for role in manifest["chronological_roles"]:
        ...
        if len(matches) > 1:
            raise USDAGrainAcquisitionError("duplicate access-ledger role")
        if matches:
            continue
        ...
        enforce_data_access(
            ...
            role=data_role,
            requesting_module="scripts.acquire_usda_grain_information_shock_tripwire",
...
def acquire(root: Path, *, execute: bool) -> dict[str, Any]:
    ...
    if receipt_path.is_file():
        ...
        if stable_hash(core) != claimed or receipt.get("bundle_id") != bundle_id:
            raise USDAGrainAcquisitionError("existing receipt drift")
        for row in receipt["files"]:
            path = Path(row["path"])
            if not path.is_file() or sha256_file(path) != row["sha256"]:
                raise USDAGrainAcquisitionError("raw artifact drift")
        return receipt

    key = load_api_key()
    if not key:
        raise USDAGrainAcquisitionError("DATABENTO_API_KEY unavailable")
    ...
    if actual_before + total > ceiling + 1e-12:
        raise USDAGrainAcquisitionError("authoritative budget exceeded")
    plan = {
        "schema": "hydra_usda_grain_information_shock_acquisition_plan_v1",
        "bundle_id": bundle_id,
    ...
    with _optional_lock(root / GLOBAL_LOCK, enabled=True):
        with _optional_lock(root / LOCAL_LOCK, enabled=True):
            ...
            if actual_now + total > ceiling + 1e-12:
                raise USDAGrainAcquisitionError("concurrent spend exhausted authority")
            ...
            for row in file_rows:
                ...
                if any(item.get("request_id") == request_id for item in ledger):
                    raise USDAGrainAcquisitionError("pre-existing spend row without sealed receipt")
            ...
            core = {
                **plan,
                "schema": "hydra_usda_grain_information_shock_acquisition_receipt_v1",
                "download_status": "DOWNLOADED",
                "files": file_rows,
                "network": network,
