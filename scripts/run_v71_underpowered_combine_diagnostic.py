from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from hydra.validation.v71_underpowered_combine_diagnostic import (
    run_underpowered_combine_diagnostic,
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the frozen V7.1 underpowered Combine diagnostic."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument(
        "--proof-registry", default="mission/state/proof_registry.json"
    )
    parser.add_argument("--output-dir", default="reports/v7_1/combine_research_0001")
    args = parser.parse_args()
    result = run_underpowered_combine_diagnostic(
        project_root=args.project_root,
        proof_registry_path=args.proof_registry,
        output_dir=args.output_dir,
    )
    primary = "eod_level_rt_breach__DLL_disabled"
    print(
        json.dumps(
            {
                "candidate_count": result["candidate_count"],
                "episode_start_count": result["episode_start_count"],
                "effective_nonoverlapping_block_count": result[
                    "effective_nonoverlapping_block_count"
                ],
                "candidate_primary": {
                    key: value["variants"][primary]
                    for key, value in result["candidate_results"].items()
                },
                "basket_primary": result["basket"]["variants"][primary],
                "result_path": result["result_path"],
                "result_sha256": result["result_sha256"],
            },
            indent=2,
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
