...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free V7.1 speed-leadership manifest."
    )
    ...
    minute, transitions, audit = load_speed_leadership_sources(root)
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_cross_clock_speed_leadership_signal_manifest_v1",
        ...
        "CONTRE": (
            "Relative clock speed can proxy generic activity and both clocks "
            "share the same prints; economic replay and the permanent tripwire remain mandatory."
        ),
    }
    ...
    result_path = output / "v71_cross_clock_speed_leadership_signal_manifest.json"
    ...
    report_path = output / "v71_cross_clock_speed_leadership_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Speed-leadership outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=155 verdict=GREEN",
            f"gate=V71_G5_SIGNAL_FREEZE preuve={displayed}#{result_hash[:8]} tests=outcome_free_generation",
            ...
            f"- Transitions: `{audit.speed_leadership_transition_count}`",
            "",
            "## CONTRE",
            "",
            str(payload["CONTRE"]),
