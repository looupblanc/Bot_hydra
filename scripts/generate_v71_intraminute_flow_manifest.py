...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the outcome-free V7.1 intraminute-flow manifest.")
    ...
    _, states, audit = load_intraminute_flow_sources(root)
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_intraminute_flow_signal_manifest_v1",
        ...
        "CONTRE": "Natural half-minute flow allocation can remain arbitrary clock geometry; economic replay and the permanent price-null tripwire remain mandatory.",
    }
    ...
    result_path = output / "v71_intraminute_flow_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    displayed = result_path.relative_to(root) if result_path.is_relative_to(root) else result_path
    report_path = output / "v71_intraminute_flow_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Intraminute-flow outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=172 verdict=GREEN",
            f"gate=V71_G8_SIGNAL_FREEZE preuve={displayed}#{result_hash[:8]} tests=outcome_free_generation",
            "budget_llm=usage_API_non_exposee/solde budget_data=87.847388/125.00_achat_phase=0 N_trials=263844 burned=1",
            "diff_validation=aucun CONTRE=la_frontiere_30_secondes_peut_etre_une_geometrie_arbitraire",
...
def _existing_archive(root: Path) -> dict[str, str]:
    ...
    for path in paths:
        if path.name == "v71_intraminute_flow_signal_manifest.json":
            continue
...
def _within_manifest_duplicates(rows: list[dict[str, Any]]) -> dict[str, str]:
    ...
    return {candidate_id: sorted(ids)[0] for ids in groups.values() for candidate_id in sorted(ids)[1:]}
