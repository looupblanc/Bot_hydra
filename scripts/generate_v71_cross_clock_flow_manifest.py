...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free limited V7.1 cross-clock manifest."
    )
    ...
    minute, pairs, audit = load_cross_clock_sources(root)
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_cross_clock_flow_signal_manifest_v1",
        "grammar_id": GRAMMAR_ID,
        "candidate_count": len(rows),
        "family_count": 1,
        "candidates_per_family": dict(Counter(row["family_id"] for row in rows)),
        ...
        "CONTRE": "Cross-clock agreement can be duplicated information from the same print stream; economic replay and the grammar tripwire remain mandatory."
    }
    ...
    result_path = output / "v71_cross_clock_flow_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    report_path = output / "v71_cross_clock_flow_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Limited cross-clock outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=150 verdict=GREEN",
            f"gate=V71_G4_SIGNAL_FREEZE preuve={result_path.relative_to(root)}#{result_hash[:8]} tests=outcome_free_generation",
            ...
            f"- Paires alignées: `{audit.aligned_availability_minute_count}`",
            "",
            "## CONTRE",
            "",
            str(payload["CONTRE"]),
