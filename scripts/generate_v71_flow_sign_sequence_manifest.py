...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the outcome-free V7.1 flow-sign manifest.")
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output-dir", default="reports/v7_1/discovery_0007")
    args = parser.parse_args()
    root = Path(args.project_root).resolve()
    specs = {row.candidate_id: row for row in candidate_specs(root)}
    _, states, audit = load_flow_sign_sequence_sources(root)
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_flow_sign_sequence_signal_manifest_v1",
        ...
        "CONTRE": "Ordinal flow-sign patterns may be ubiquitous combinatorial states rather than economic mechanisms; economic replay and the permanent tripwire remain mandatory.",
    }
    ...
    result_path = output / "v71_flow_sign_sequence_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    displayed = result_path.relative_to(root) if result_path.is_relative_to(root) else result_path
    report_path = output / "v71_flow_sign_sequence_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Flow-sign sequence outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=168 verdict=GREEN",
            f"gate=V71_G7_SIGNAL_FREEZE preuve={displayed}#{result_hash[:8]} tests=outcome_free_generation",
            "budget_llm=usage_API_non_exposee/solde budget_data=87.847388/125.00_achat_phase=0 N_trials=263814 burned=1",
            "diff_validation=aucun CONTRE=les_sequences_de_signes_peuvent_etre_une_geometrie_combinatoire",
...
def _existing_archive(root: Path) -> dict[str, str]:
    paths = (
        ...
        root / "reports/v7_1/discovery_0006/v71_trade_size_composition_signal_manifest.json",
    )
