...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free V7.2 executed-price occupancy manifest."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument("--output-dir", default="reports/v7_2/discovery_0012")
    args = parser.parse_args()
    root = Path(args.project_root).resolve()
    specs = {row.candidate_id: row for row in candidate_specs(root)}
    _, states, audit = load_executed_price_occupancy_sources(root)
    ...
    payload: dict[str, Any] = {
        "schema": "hydra_v7_2_executed_price_occupancy_signal_manifest_v1",
        ...
        "proof_registry_reservation": {
            "event_id": "v7_2_executed_price_occupancy_structural_tripwire_reservation_0012",
            "entry_hash": "5f4544d4c786a94a257872ff457f3dcc58a0152fb84c74b1847f53a1adeaa22b",
            "raw_global_N_trials_after": 265_347,
        },
        "new_data_purchase_count": 0,
        "authoritative_q4_access_count_before_and_after": 1,
        "protected_holdout_access_count_delta": 0,
        "outbound_order_count": 0,
        "CONTRE": (
            "Executed-price occupancy can remain a contemporaneous transformation "
            "of range, volatility and minute direction; economic walk-forward and "
            "the frozen price-world tripwire are mandatory."
        ),
    }
    ...
    result_path = output / "v72_executed_price_occupancy_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    displayed = (
        result_path.relative_to(root) if result_path.is_relative_to(root) else result_path
    )
    report_path = output / "v72_executed_price_occupancy_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.2 — Executed-price occupancy outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=206 verdict=GREEN",
            f"gate=V72_G12_SIGNAL_FREEZE preuve={displayed}#{result_hash[:8]} tests=outcome_free_generation",
            "budget_llm=usage_API_non_exposee/solde budget_data=87.847388/125.00_achat_phase=0 N_trials=265347 burned=1",
            "diff_validation=aucun CONTRE=loccupation_peut_encoder_seulement_la_geometrie_contemporaine",
...
def _existing_archive(root: Path) -> dict[str, str]:
    ...
    for path in paths:
        if path.name == "v72_executed_price_occupancy_signal_manifest.json":
            continue
