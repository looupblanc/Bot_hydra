...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free V7.1 opportunity-density manifest."
    )
        ...
        rows.append(
            {
                ...
                "powered_for_DSR_BH_forecast": len(candidate_signals) >= 320,
            }
        )
    duplicates = _within_manifest_duplicates(rows)
    for row in rows:
        row["within_manifest_duplicate_of"] = duplicates.get(
            str(row["candidate_id"])
        )
    family_counts = Counter(str(row["family_id"]) for row in rows)
    payload: dict[str, Any] = {
        "schema": "hydra_v7_1_opportunity_density_signal_manifest_v1",
        "grammar_id": GRAMMAR_ID,
        "candidate_count": len(rows),
        "family_count": len(family_counts),
        "candidates_per_family": dict(sorted(family_counts.items())),
        "signal_count": sum(int(row["signal_count"]) for row in rows),
        "powered_candidate_count_forecast": sum(
            bool(row["powered_for_DSR_BH_forecast"]) for row in rows
        ),
        ...
        "CONTRE": (
            "More opportunities can merely add weak trades; signal density is "
            "not economic evidence and cannot bypass walk-forward transfer."
        ),
    }
    ...
    result_path = output / "v71_opportunity_density_signal_manifest.json"
    _atomic_json(result_path, payload)
    result_hash = _sha256(result_path)
    report_path = output / "v71_opportunity_density_signal_manifest.md"
    report = "\n".join(
        [
            "# HYDRA V7.1 — Opportunity-density outcome-free manifest",
            "",
            "[HYDRA-V7] phase=4 step=130 verdict=GREEN",
            f"gate=V71_G2_SIGNAL_FREEZE preuve={result_path.relative_to(root)}#{result_hash[:8]} tests=outcome_free_generation",
            "budget_llm=usage_API_non_exposee/solde budget_data=87.847388/125.00_achat_phase=0 N_trials=262228 burned=1",
            "diff_validation=aucun CONTRE=une_densite_de_signaux_superieure_ne_prouve_aucune_esperance",
            ...
            f"- Prévision >=320 signaux: `{payload['powered_candidate_count_forecast']}`",
...
def _existing_archive(root: Path) -> dict[str, str]:
    path = root / "reports/v7_1/discovery/v71_signal_manifest.json"
