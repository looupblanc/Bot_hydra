...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the outcome-free V7 grammar 0001 signal manifest."
    )
    parser.add_argument("--project-root", default=".")
    parser.add_argument(
        "--preregistration",
        default="WORM/v7-grammar-0001-hypotheses-2026-07-12.json",
    )
    parser.add_argument(
        "--output", default="reports/v7/phase4/grammar0001_signal_manifest.json"
    )
    args = parser.parse_args()
    root = Path(args.project_root).resolve()
    preregistration = root / args.preregistration
    if _sha256(preregistration) != PREREGISTRATION_SHA256:
        raise RuntimeError("V7 grammar preregistration hash mismatch")
    bars = load_v7_market_bars(root)
    signals = generate_signal_population(
        bars, graveyard_path=root / "mission/state/graveyard.db"
    )
    payload = {
        "schema": "hydra_v7_grammar_0001_signal_manifest_v1",
