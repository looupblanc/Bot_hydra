...
def test_timeline_tripwire_requires_own_history_enrichment() -> None:
    rows = [_pair_row(real_win=index < 90, index=index) for index in range(100)]
    result = account_timeline_paired_tripwire(rows, _tripwire_gate())
...
def test_timeline_tripwire_rejects_control_dominance() -> None:
...
def test_timeline_tripwire_enforces_consistency_non_deterioration() -> None:
    rows = [
        _pair_row(
            real_win=index < 90,
            index=index,
...
def _complete_pair_row() -> dict:
    ...
    return {
        **_pair_row(real_win=True),
        "real_policy": {
            "policy_id": "real-timeline-1",
            "structural_fingerprint": "real-timeline-structure-1",
        },
        "matched_control_policy": {
            "policy_id": "control-timeline-1",
            "structural_fingerprint": "control-timeline-structure-1",
        },
        "behavioral_fingerprint": "timeline-behavior-1",
        "real_evaluation": {
...
def test_final_result_never_promotes_development_timeline() -> None:
...
def test_family_failure_blocks_research_despite_profitable_policy() -> None:
    result = account_timeline_final_result(
        _prereg(),
        population_summary={"manifest_hash": "manifest", "real_policy_count": 1},
        screen_summary={"survivor_count": 1},
        ...
        tripwire={
            ...
            "evidence_strength": "ARTEFACT",
        },
        elapsed_seconds=2.0,
        phase_seconds={"paired_account_replay": 1.5},
    )
    assert result["diagnostic_account_gate_pass_count"] == 1
    assert result["account_research_candidate_count"] == 0
    assert result["combine_path_diagnostic_count"] == 0
    assert result["next_action"].startswith("TOMBSTONE_EXACT_0012")
