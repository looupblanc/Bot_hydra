...
def test_state_tripwire_requires_role_map_enrichment() -> None:
    rows = [_pair_row(real_win=index < 90, index=index) for index in range(100)]
    result = account_state_paired_tripwire(rows, _tripwire_gate())
...
def test_state_tripwire_rejects_control_dominance() -> None:
...
def test_state_tripwire_enforces_consistency_non_deterioration() -> None:
    rows = [
        _pair_row(
            real_win=index < 90,
            index=index,
...
def _complete_pair_row() -> dict:
    ...
    return {
        **_pair_row(real_win=True),
        "real_policy": {
            "policy_id": "real-state-1",
            "structural_fingerprint": "real-state-structure-1",
        },
        "matched_control_policy": {
            "policy_id": "control-state-1",
            "structural_fingerprint": "control-state-structure-1",
        },
        "behavioral_fingerprint": "state-behavior-1",
        "real_evaluation": {
...
def test_final_result_never_promotes_development_state_router() -> None:
    prereg = {
...
def test_family_failure_blocks_account_research_even_when_policy_is_profitable() -> None:
    prereg = {
        "campaign_id": "hydra_economic_evolution_account_state_router_0011",
        "account_gate": {
            "minimum_normal_median_net_usd": 0.0,
    ...
    assert result["next_action"].startswith("TOMBSTONE_EXACT_0011")
