...
def _tripwire_gate() -> dict:
    return {
        ...
        "maximum_mll_breach_rate_deterioration": 0.02,
        "maximum_NULL_RATIO": 0.8,
        "net_evidence_p_value": 0.05,
    }


def _pair_row(*, real_win: bool, index: int = 0) -> dict:
    ...
    return {
        ...
        "paired_delta": {
            ...
            "normal_consistency_pass_rate": 0.0,
            "stressed_consistency_pass_rate": 0.0,
        },
    }


def test_paired_tripwire_requires_net_account_enrichment() -> None:
    rows = [
        _pair_row(real_win=index < 90, index=index) for index in range(100)
    ]
    result = paired_account_tripwire(rows, _tripwire_gate())
...
def test_paired_tripwire_rejects_control_dominance() -> None:
    rows = [
        _pair_row(real_win=index < 40, index=index) for index in range(100)
    ]
    result = paired_account_tripwire(rows, _tripwire_gate())
...
def test_final_result_keeps_development_evidence_non_promotional() -> None:
    prereg = {
        ...
        "account_gate": {
            ...
            "maximum_positive_component_share": 0.7,
